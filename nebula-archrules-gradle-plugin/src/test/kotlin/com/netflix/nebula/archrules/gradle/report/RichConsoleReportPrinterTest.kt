...
internal class RichConsoleReportPrinterTest {
    @Test
    fun `test printSummary`() {
        ...
        val instance = RichConsoleReportPrinter(output, RichConsoleReportPrinter.calculateMaxRuleNameLength(results))
    ...
    fun `test printSummary skipPassing with failures`() {
        ...
        val instance = RichConsoleReportPrinter(output, RichConsoleReportPrinter.calculateMaxRuleNameLength(results))
        instance.printSummary(mapOf(rule to results), true, false)
        assertThat(output.getOutput())
            .contains("RuleClass")
            .contains("RuleName")
    }
    ...
    fun `test printSummary skipPassing no failures but infoLogging on`() {
        ...
        val results = listOf(RuleResult(rule, "message", RuleResultStatus.PASS))
        val instance = RichConsoleReportPrinter(output, RichConsoleReportPrinter.calculateMaxRuleNameLength(results))
        instance.printSummary(mapOf(rule to results), true, true)
        assertThat(output.getOutput())
            .contains("RuleClass")
            .contains("RuleName  MEDIUM     (No failures)")
    }
    ...
    fun `test printSummary empty results`() {
        val output = MockStyledTextOutput()
        val instance = RichConsoleReportPrinter(output, 1)
